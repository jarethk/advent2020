...
while (rq.length > 0) {
    let rtp = rq.shift();
    if (rtp === "8") {
        rules[rtp] = rules[rules[rtp]] + "+";
    } else {
        // kind-of brute force, but we know from the message sizes that there won't be more than a few repeats
        if (rtp === "11") {
            rules[rtp] = "42 31 | 42 42 31 31 | 42 42 42 31 31 31 | 42 42 42 42 31 31 31 31"
        }
        rules[rtp] = rules[rtp]
            .split(" ")
            .map(c => c === "|" ? "|" : rules[c])
